...
3. **下一步（UI）**：`mvp/api/` FastAPI 桥真实实现（当前仅端点契约文档；前端设 `VITE_API_BASE` 即从 Mock 切 Http，不改码）→ 真实后端连通（进度 WS + 取消 + 预览 `extracted_path`）→ Electron 打包/真机验收 → 性能基准（10/60/120min）+ Confidence 标定。
...
- **2026-08-26 追加**：`infrastructure/logging.py` 统一日志（stderr + `RotatingFileHandler` 10MB×5 写 `mvp/logs/video_locator.log`，结构化 `时间 级别 module session 消息`，`contextvars` session_id）；`device/__init__.py` 记 backend requested/selected/fallback/device；`app/locator_service.py` 记 index/analysis/locate 生命周期 + 异常(exception)。`mvp/api/README.md` = FastAPI 桥端点契约（未实现）。
...
- **UI（第 8 项）= Vue3+TS+Electron，Stage 1 初始代码已交付（2026-08-26）**；下一步 = `mvp/api/` FastAPI 桥真实实现（前端设 `VITE_API_BASE` 切 Http，不改码）→ 真实后端连通 → Electron 打包/真机验收 → 性能基准 + Confidence 标定。
...
8. **PySide6 UI → 转向 = Vue3+TS+Electron 桌面工作台（2026-08-26 交付 Stage 1）**。`mvp/ui/`（Vue3+TS+Vite+Pinia+router+Electron 壳）+ 自研 Fluent 组件 + `ServiceAPI`(Mock 适配器独立跑通 / `HttpServiceAdapter` 预留 `/api`+WS) + `services/types.ts` 镜像后端 JSON + 6 页面 + 设计文档。**剩余**：`mvp/api/` FastAPI 桥真实实现（设 `VITE_API_BASE` 切 Http 不改 UI 码）→ 真实后端连通（进度 WS+取消+预览 `extracted_path`）→ Electron 打包/真机验收（`ui/docs/DESKTOP.md`）。
