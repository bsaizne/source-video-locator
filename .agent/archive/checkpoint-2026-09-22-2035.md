...
M .agent/STATE.md
 M .agent/TODO.md
 M AGENTS.md
?? mvp/benchmark/user_case/semantic_signal/RESEARCH_PROPOSAL_TRAINING.md
?? mvp/scripts/probe_m7_v2.py
?? mvp/scripts/probe_training_t1a.py
?? mvp/scripts/probe_training_t1b.py
```

### Recent Commits

```text
ff71db6 fix(ui): vendor-neutral GPU device labels + filter prefs by available_devices
e7439e5 fix(mac): ad-hoc codesign + ditto rezip to fix 'app is damaged' on open
a9710f2 ci: remove duplicated macos-package job definition (duplicate YAML key rejected by GitHub)
cba35d5 ci(mac): fix release publish — notes to committed file (--notes-file), no stderr suppression, idempotent if/else
da1153e ci(mac): publish signed-off zip to GitHub Releases (rolling tag mac-alpha, prerelease)
```
...
<!-- 结构修复：此处合并了两段重复的 `## Current Task`。上面一段是原文 L1 起的最新记录（2026-09-06），下面一段是原文 L137 起的更早记录（2026-09-05 及以前）。 -->
