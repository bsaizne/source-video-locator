...
- **UI（第 8 项）= Vue3+TS+Electron，Stage 1 初始代码已交付（2026-08-26）**；`mvp/api/` FastAPI 桥**已实现**（2026-08-26）。下一步 = `HttpServiceAdapter` 从 Mock 切真实 API（前端设 `VITE_API_BASE`，不改 UI 页）→ 真实后端连通（进度 WS + 取消 + 预览 `extracted_path`）→ Electron 打包/真机验收 → 性能基准 + Confidence 标定。
...
8. **PySide6 UI → 转向 = Vue3+TS+Electron 桌面工作台（2026-08-26 交付 Stage 1）**。`mvp/ui/`（Vue3+TS+Vite+Pinia+router+Electron 壳）+ 自研 Fluent 组件 + `ServiceAPI`(Mock/Http 双适配器) + `services/types.ts` 镜像后端 JSON + 6 页面 + 设计文档。**`mvp/api/` FastAPI 桥已实现（2026-08-26）**（见 STATE.md）；**`HttpServiceAdapter` 已从 Mock 切真实 FastAPI（2026-08-26）**：Mock/HTTP 双模式（`VITE_BACKEND_MODE`）、Backend Connection 三态检测（`GET /api/health`）、统一错误处理（`BackendUnavailableError`/500 detail）、vitest 14 项、浏览器实测 Home Connection=Connected；关键落差口径见 STATE.md（`getIndexStatus/meta/loadResults/onProgress/cancel` 在 Http 用诚实占位/粗阶段提示；`/api/export` 只收 `output_dir`）。**剩余**：真实后端连通（进度 WS + 取消 + 预览 `extracted_path`）→ Electron 打包/真机验收（`ui/docs/DESKTOP.md`）→ 性能基准 + Confidence 标定。
