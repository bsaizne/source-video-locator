...
**MVP 产品化 — Stage 1（编码）进行中**：已完成第 1~7 项（media → domain/infrastructure → device/feature_store → retrieval/clustering/ranking → localization/confidence → engine/segment → **application service**），**下一项 = 第 8 项 PySide6 UI**。算法研究 Phase 1~19 已冻结收尾。
