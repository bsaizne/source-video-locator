...
>
> **硬件/开发状态（2026-08-26 统一）**：H1 Windows CPU = **IMPLEMENTED**；H2 Windows AMD / DirectML = **IMPLEMENTED**；H3 macOS Apple Silicon / MPS = **GO / POC completed**；H4 Windows NVIDIA / CUDA = **PENDING**（下一阶段 POC 准备）；UI（第 8 项 PySide6）= **PAUSED**。
...
1. **下一阶段：H4 Windows NVIDIA / CUDA POC（PENDING）**——能力探测（torch.cuda + 可用设备）→ 若可用，`CUDABackend` 一致性验证（特征/检索/定位/置信与 CPU 对比，cos>0.999 量级）→ 性能基准；不支持则如实标记 `H4_CUDA_BLOCKED` 继续 CPU。CUDA 不进当前实施阶段。
...
- **H4 Windows NVIDIA / CUDA POC（下一阶段准备，= PENDING）**：能力探测（torch.cuda + 可用设备）→ 若可用，`CUDABackend` 一致性验证（特征/检索/定位/置信与 CPU 对比，cos>0.999 量级）→ 性能基准（10/60/120min）；不支持则如实标记 `H4_CUDA_BLOCKED` 继续 CPU。
