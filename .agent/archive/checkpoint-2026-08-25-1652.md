...
**MVP 产品化 — Stage 1（编码）进行中**：已完成第 1~4 项（media → domain/infrastructure → device/feature_store → retrieval/clustering/ranking），**下一项 = 第 5 项 `engine/localization` + `engine/confidence`**。算法研究 Phase 1~19 已冻结收尾。

---

## Current Task

1. `engine/localization`（longest_run + montage 检测分支）+ `engine/confidence/ConfidenceEngine`。
2. `engine/segment`（edited shot 切分，定义查询单元）。
3. application service（用例编排 + Result 持久化）。
4. **PySide6 UI**（Home→Index→Results→Export + 预览 + 手动修正 + 批量导出）。
5. 打包（PyInstaller）+ 真机验收 + **性能基准**（10/60/120min）+ **Confidence 标定**（`CONFIDENCE_DESIGN.md` 权重/阈值用真实数据校准）。
...
- `mvp/src/` 已构建模块树：`media/ffmpeg`（FFmpegIO）、`domain`、`infrastructure`、`device/`（CPUBackend）、`engine/feature_store`（FeatureStore）、`engine/common|retrieval|clustering|ranking`、`engine/candidates`。
- 测试：`mvp/tests/` 4 模块 29 项 unittest；`mvp/scripts/smoke_*.py` 冒烟（media 23 / device+feature_store 25 / retrieval+ranking 6）全过。
- 冻结栈已贯通：FFmpegIO 抽帧 → CPUBackend(DINOv2 CPU) 特征 → FeatureStore 落盘索引 → retrieval/clustering/ranking 产出候选窗。**尚未接上 finloc/confidence/segment/UI**。
...
- **第 5 项**：`engine/localization`（longest_run + montage 检测分支；只 REUSE Phase14C longest_run + per-orig max-over-query coverage；**禁** per-query argmax / voting / cut-aware / temporal alignment / 改 similarity·ranking）+ `engine/confidence/ConfidenceEngine`（rank←rank_margin/n_reps/qcov/consistency/sim_std/scene_div + localization best_cover/span stability/coverage quality → HIGH/MEDIUM/LOW + score + reasons + hard_flags；禁调阈值/禁 sweep）。
- 之后：第 6 项 `engine/segment`（edited shot 切分，定义无 GT 查询单元）→ 第 7 项 application service → 第 8 项 PySide6 UI → 第 9 项 打包验收 + 性能基准 + Confidence 标定。
