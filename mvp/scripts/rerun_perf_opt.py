...
独立结果文件 work/rerun_<case>_perfopt.results.json（历史结果不覆盖约定；
...
def main():
    ...
    print("seg_twopass_enabled =", cfg.pipeline.seg_twopass_enabled,
          ...
          "dml_batch_size =", cfg.device.dml_batch_size, flush=True)
    for name, paths in CASES.items():
        if only and name != only:
            continue
        out = BENCH / "work" / f"rerun_{name}_perfopt.results.json"
        if out.exists() and not os.environ.get("SVL_FORCE_RERUN"):
            print(f"  {name}: skip (result exists)", flush=True)
            continue
        print(f"\n=== {name} perf opt ===", flush=True)
        t0 = time.monotonic()
        try:
