"""Phase 24-2 · 索引密度探针 M4 —— 8fps 密帧 vs 1fps 稀疏索引的判别力实测。
...
# (pair, pid, 编辑段, 正确窗, 干扰窗, 说明)
# 干扰窗选「真正同质/难分」的邻窗(test4 同质紧邻 / t3r12 另一重复实例 / p08 兄弟 / p26 夜阳台)
CASES = [
    ("2mkv", "p08", (12.6, 14.3), (1106, 1121), (1041, 1061),
     "兄弟机位: 瞭望塔(真值) vs 士兵特写(兄弟, 机位差异)"),
    ("2mkv", "p26", (76.0, 78.0), (2786, 2822), (1766, 1813),
     "夜读(真值) vs 夜阳台(干扰, 编辑上下文错误区)"),
    ("test3", "t3r12", (64.0, 65.5), (454, 481), (481, 488),
     "精灵王重复镜头: 真值场景43-45 vs 另一重复实例场景46"),
    ("test4", "t4r01", (1.25, 2.5), (3329, 3355), (3355, 3398),
     "同质滑梯: 真值353-355 vs 紧邻同质干扰"),
]
...
def main() -> int:
    ...
    out = WORK / "semantic_signal_M4_results.json"
