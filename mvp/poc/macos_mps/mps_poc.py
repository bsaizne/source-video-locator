...
def correctness(frames: list[np.ndarray], model, dim: int, *, device: str,
    ...
    cpu_feats, cpu_ms = embed_frames(frames, model, dim, "cpu", batch_size)
    mps_feats, mps_ms = embed_frames(frames, model, dim, device, batch_size)
...
def throughput(frames: list[np.ndarray], model, dim: int, *, device: str,
               batch_size: int) -> dict:
    feats, ms = embed_frames(frames, model, dim, device, batch_size)
    n = len(frames)
    return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.quick:
        ...
        feats, _ = embed_frames(frames, model, dim, "cpu", batch_size=8)
        ...
        print(f"[selfcheck] cpu L2 norm ok={l2_ok} (max_dev="
