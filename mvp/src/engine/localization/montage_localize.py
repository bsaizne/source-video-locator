"""engine.localization.montage_localize — 蒙太奇多段定位（产品化，2026-08-27）。
...
研究验证（mvp/benchmark/user_case/montage_research/FINDINGS.md）：GT recall 基线 2/7 → 86%，
子 span 精度 ~94%，子 span 对 GT 对齐。``localize()`` 无 IO、确定性；只依赖冻结的
...
class MontageSpan:
    ...
    edited_interval: tuple[float, float]       # 该子镜头在编辑片的时间区间
...
class MontageLocalizer:
    ...
    def localize(self, ed_feats: np.ndarray, ed_times: np.ndarray,
        ...
        for (c0, c1, cnt, cidx) in sig:
            ...
            both_low = (cover < self.weak_cover
                        and (bsim is None or bsim < self.weak_sim))
            ...
            built.append(ms)
        spans = [s for s in built if not s.dropped_weak]
        return MontageLocalizationResult(
            ...
            n_dropped_weak=sum(1 for s in built if s.dropped_weak))
    ...
    def _locate_cluster(self, cidx, cluster_best_t, q, tq, bundle, low, high):
        ...
        # 子 span 用「簇最佳匹配范围 ∪ finloc run」覆盖子镜头完整区域（保住 GT recall）；
        # 不收紧到 tight_span（峰值居中会裁掉子镜头边界），短镜头收紧只在干净段做（app 层）。
